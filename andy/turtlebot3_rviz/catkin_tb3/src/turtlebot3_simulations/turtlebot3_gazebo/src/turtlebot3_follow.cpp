...
//set the lookup tf
	...
	try{
		listener.waitForTransform("base_footprint", "target", ros::Time(0), ros::Duration(10.0));
		listener.lookupTransform("base_footprint", "target",
  ...
  switch(turtlebot3_state_num)
  {
    case GET_TB3_DIRECTION:
      ...
      {
        prev_tb3_pose_ = tb3_pose_;
        turtlebot3_state_num = TB3_TARGET_TURN;
      }
			if (dist <= 0.05)
			{
				updatecommandVelocity(0.0, 0.0);
    ...
    case TB3_DRIVE_FORWARD:
			ang = 0.4 * ANGULAR_VELOCITY * atan2(transform.getOrigin().y(),
																						transform.getOrigin().x());
      updatecommandVelocity((LINEAR_VELOCITY), ang);
      turtlebot3_state_num = GET_TB3_DIRECTION;
      break;
    ...
    case TB3_LEFT_TURN:
      ...
      break;
		case TB3_TARGET_TURN:		
      if (fabs(prev_tb3_pose_ - tb3_pose_) >= escape_range_)
        turtlebot3_state_num = GET_TB3_DIRECTION;
      else{
				ang = 0.5 * ANGULAR_VELOCITY * atan2(transform.getOrigin().y(),
																						transform.getOrigin().x());
				if (ang <= 0.2)
					ang = ANGULAR_VELOCITY;	
        updatecommandVelocity(0.0, ang);
			}
      break;
    default:
      turtlebot3_state_num = GET_TB3_DIRECTION;
