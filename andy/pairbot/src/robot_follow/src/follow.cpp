...
#include "turtlebot3_gazebo/pairbot_depth.h"
#include <ros/ros.h>
...
bool Turtlebot3Drive::init()
{
  ...
  depth_scan_sub_  = nh_.subscribe("/Zed_sim/depth/depth_registered", 10, &Turtlebot3Drive::depthScanMsgCallBack, this);  
...
float check_obstacle(float depth, int i, float robot_width)
{
	...
	float avoid_width = (robot_width / pixel_length)*1.2 ;//the range that need to avoid(1.2 to make the range bigger to avoid)
